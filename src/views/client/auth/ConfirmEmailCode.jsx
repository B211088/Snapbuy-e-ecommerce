...
const ConfirmEmailCode = () => {
  ...
  const handleReSendMail = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        notifyError(response.message || "Gửi mã thất bại!");
        setLoading(false);
        return;
      }
    } catch (err) {
      notifyError("Có lỗi khi gửi mã xác nhận:", err.message);
      setLoading(false);
      return;
    }
  };
  ...
  const handleRegister = async () => {
    try {
      ...
      console.error(error);
      notifyError("Tạo tài khoản thất bại", 0);
      return false;
    }
  };
  ...
  const handleConfirmEmail = async () => {
    ...
    if (otpCode.length !== 6) {
      notifyWarning("Vui lòng nhập đủ 6 số mã xác nhận");
      return;
    }
    try {
      ...
      if (response.success) {
        notifySuccess("Xác nhận email thành công!");
      ...
      notifyError(response.message || "Xác nhận thất bại");
    } catch (error) {
      notifyError("Lỗi server, vui lòng thử lại sau!: ", error.message);
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="w-full container-minus-headerflexible flex items-center justify-center">
        <div
          className={`w-[32%] max-w-[420px] min-w-[340px] tl:min-w-[400px]  rounded-[5px]  ${
