const apiServer = import.meta.env.VITE_BACKEND_API_URL;

export const apiUrl = apiServer;

export const LOCAL_STORAGE_TOKEN_NAME = "accessToken";
export const LOCAL_STORAGE_USER = "userId";

export const SET_AUTH = "SET_AUTH";
export const UPDATE_AUTH = "UPDATE_AUTH";

export const SET_PROVINCES = "SET_PROVINCES";
export const SET_DISTRICTS = "SET_DISTRICTS";
export const SET_VILLAGES = "SET_VILLAGES";
export const SET_ADDRESSES = "SET_ADDRESSES";
export const ADD_ADDRESS = "ADD_ADDRESS";
export const UPDATE_ADDRESS = "UPDATE_ADDRESS";
export const DELETE_ADDRESS = "DELETE_ADDRESS";

export const SET_ROLE = "SET_ROLE";
export const SET_AVATAR = "SET_AVATAR";
export const SET_AUTH_LOADING = "SET_AUTH_LOADING";

export const SET_AUTH_ADMIN = "SET_AUTH_ADMIN";

export const SET_ALL_CATEGORIES = "SET_ALL_CATEGORIES";
export const ADD_CATEGORY = "ADD_CATEGORY";
export const UPDATE_CATEGORY = "UPDATE_CATEGORY";
export const REMOVE_CATEGORY = "REMOVE_CATEGORY";
export const SET_ALL_SUB_CATEGORIES = "SET_ALL_SUB_CATEGORIES";
export const ADD_SUB_CATEGORY = "ADD_SUB_CATEGORY";
export const UPDATE_SUB_CATEGORY = "UPDATE_SUB_CATEGORY";
export const REMOVE_SUB_CATEGORY = "REMOVE_SUB_CATEGORY";
export const SET_ALL_ATTRIBUTES = "SET_ALL_ATTRIBUTE";
export const ADD_MULTIPLE_ATTRIBUTES = "ADD_MULTIPLE_ATTRIBUTES";
export const ADD_ATTRIBUTE = "ADD_ATTRIBUTE";
export const UPDATE_ATTRIBUTE = "UPDATE_ATTRIBUTE";
export const REMOVE_ATTRIBUTE = "REMOVE_ATTRIBUTE";
export const SET_ALL_SUBCATEGORY_ATTRIBUTES = "SET_ALL_SUBCATEGORY_ATTRIBUTES";
export const ADD_ONE_SUBCATEGORY_ATTRIBUTE = "ADD_ONE_SUBCATEGORY_ATTRIBUTE";
export const ADD_MULTIPLE_SUBCATEGORY_ATTRIBUTES =
  "ADD_MULTIPLE_SUBCATEGORY_ATTRIBUTES";
export const UPDATE_SUBCATEGORY_ATTRIBUTES = "UPDATE_SUBCATEGORY_ATTRIBUTES";
